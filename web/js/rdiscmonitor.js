CP.rdisc_monitor_4 = {
    ...
    ,init               : function() {
        CP.rdisc_monitor_4.defineStores();
        var monitorPanel = CP.rdisc_monitor_4.monitorPanel();
        var obj = {
    ...
    ,autoRefreshCallback : function(sentAutomatically) {
        var store = Ext.getStore("rdisc_monitor_summary_store");
        if(store) {
            store.doAutoRefresh(store, sentAutomatically);
        }
        
        var store = Ext.getStore("rdisc_monitor_intf_store");
        if(store) {
            store.doAutoRefresh(store, sentAutomatically);
        }

        var store = Ext.getStore("rdisc_monitor_stat_store");
        if(store) {
            store.doAutoRefresh(store, sentAutomatically);
        }
   }
    ...
    ,defineStores       : function() {
        ...
        Ext.create("CP.WebUI4.Store", {
            storeId     : "rdisc_monitor_summary_store"
            ,autoLoad   : false
            ,proxy      : {
                type            : "ajax"
                ,url            : "/cgi-bin/rdiscmonitor.tcl"
                ,extraParams    : {
            ...
            ,autoRefreshSuccess : function (jsonResult) {
                var panel = Ext.getCmp("rdisc_monitorPanel");
                if (panel) {
        ...
        Ext.create("CP.WebUI4.Store", {
            storeId     : "rdisc_monitor_intf_store"
            ,autoLoad   : false
            ,fields     : [
                "intf"
                ,"row"
            ...
            ,autoRefreshSuccess : function (jsonResult) {
                ...
                if (store) {
                    var jsonData = Ext.decode(jsonResult.responseText); 
                    if (jsonData && jsonData.data && jsonData.data.rdisc_mon_intf) {
                        store.loadData(jsonData.data.rdisc_mon_intf);
                    } 
                }
            }                        
        ...
        Ext.create("CP.WebUI4.Store", {
            storeId     : "rdisc_monitor_stat_store"
            ,autoLoad   : false
            ,fields     : [
                "intf"
                ,"row"
            ...
            ,autoRefreshSuccess : function (jsonResult) {
                ...
                if (store) {
                    var jsonData = Ext.decode(jsonResult.responseText); 
                    if (jsonData && jsonData.data && jsonData.data.rdisc_mon_stat) {
                        store.loadData(jsonData.data.rdisc_mon_stat);
                    } 
                }
            }                        
...
//monitorPanel
    ,monitorPanel           : function() {
        var monitorPanel = Ext.create("CP.WebUI4.DataFormPanel", {
            id          : "rdisc_monitorPanel"
            ,margin     : "0 24 0 24"
            ,listeners  : {
                afterrender : CP.rdisc_monitor_4.doLoad
            }
            ,items      : [
                CP.rdisc_monitor_4.get_reload_button()
                ,CP.rdisc_monitor_4.get_summary_set()
                ,CP.rdisc_monitor_4.get_interfaces_set()
                ,CP.rdisc_monitor_4.get_statistics_set()
            ]
        });
        return monitorPanel;
    }
    ...
    ,doLoad                 : function() {
        ...
        CP.UI.startAutoRefresh(CP.rdisc_monitor_4.autoRefreshCallback);
    }
    ...
    ,doReload                 : function() {
        ...
        // the "refresh" button.
        CP.rdisc_monitor_4.autoRefreshCallback(false);
    }        
        ...
        return [
            {
                ...
                text            : "Property"
                ,dataIndex      : "field0"
                ,width          : CP.rdisc_monitor_4.GRID_FIELD_WIDTH
                ,sortable       : false
                ,menuDisabled   : true
                ,groupable      : false
                ...
                text            : "Value"
                ,dataIndex      : "value0"
                ,width          : CP.rdisc_monitor_4.GRID_VALUE_WIDTH
                ,sortable       : false
                ,menuDisabled   : true
                ,groupable      : false
                ,renderer       : function(v, meta, rec) {
    ...
    ,get_interfaces_set     : function() {
        var grid = {
            xtype               : "cp4_grid"
            ,id                 : "rdisc_monitor_intf_grid"
            ,width              : CP.rdisc_monitor_4.GRID_WIDTH
            ,height             : CP.rdisc_monitor_4.GRID_HEIGHT
            ,margin             : 0
            ,forceFit           : true
            ,autoScroll         : true
            ,store              : Ext.getStore("rdisc_monitor_intf_store")
            ,columns            : CP.rdisc_monitor_4.get_grid_cm()
            ,columnLines        : false
            ,draggable          : false
    ...
    ,get_statistics_set     : function() {
        var grid = {
            xtype               : "cp4_grid"
            ,id                 : "rdisc_monitor_stat_grid"
            ,width              : CP.rdisc_monitor_4.GRID_WIDTH
            ,height             : CP.rdisc_monitor_4.GRID_HEIGHT
            ,margin             : 0
            ,forceFit           : true
            ,autoScroll         : true
            ,store              : Ext.getStore("rdisc_monitor_stat_store")
            ,columns            : CP.rdisc_monitor_4.get_grid_cm()
            ,columnLines        : false
            ,draggable          : false
            ,enableColumnMove   : false
