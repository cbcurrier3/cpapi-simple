...
Ext.apply( Ext.form.field.VTypes,{
    DNSServer: function(val, field) {
            only_digit_regex=/^\d$/;
            first_char=val.charAt(0);
            if(first_char.match(only_digit_regex)){
                return val.match(webui.ip_regex);
            }else if(first_char.match(' ') || first_char.match('')){
                return true;
			}else{
				return false;
			}		
    }
	,DNSServerText : 'Enter a valid ip address.'
	,DomainName: function(val, field) {
            only_digit_regex=/^\d$/;
            first_char=val.charAt(0);
            if(first_char.match(only_digit_regex)){
                return !(val.match(webui.ip_regex));
	...
	,DomainNameText : 'looks more like a numeric IPv4 address than a domain name.' ,
    ...
    hostname: function( val ){
		var hostNameRule = /^[a-zA-Z0-9]([a-zA-Z0-9\_]|[\.][a-zA-Z0-9]|[\_][a-zA-Z0-9]|[\-][a-zA-Z0-9\-]){0,62}$/ ;
		var hostNameRule2 = /\-$/; // We don't allow hostnames ends with hyphen.
        var isValidHost = hostNameRule.test( val ) && !hostNameRule2.test(val); 
        var isValidIp = cpVTypes_ipv4Rule.test( val ); // Consider removing  'isValidIp' - not necessarilly relevant
    ...
    hostnameText: 'Name must consist of 63 alphanumerics, numbers, "-", and "."',
     ...
     * Permitted hostname labels to start with digits. 
     * No other symbols, punctuation characters, or white space are permitted.
     * Hostname field can also contain an IPv4 or IPv6 address.
     * e.g. comp.checkpoint.com, 172.16.254.1, 1111::1110
     */
    hostnameIPv6: function( val ){
		var hostNameRule = /^[a-zA-Z0-9]([a-zA-Z0-9]|[\.][a-zA-Z0-9]|[\-][a-zA-Z0-9\-]){0,62}$/ ;
		var hostNameRule2 = /\-$/; // We don't allow hostnames ends with hyphen.
		var hostNameRule3 = /^\s*((([0-9A-Fa-f]{1,4}:){7}([0-9A-Fa-f]{1,4}|:))|(([0-9A-Fa-f]{1,4}:){6}(:[0-9A-Fa-f]{1,4}|((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){5}(((:[0-9A-Fa-f]{1,4}){1,2})|:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){4}(((:[0-9A-Fa-f]{1,4}){1,3})|((:[0-9A-Fa-f]{1,4})?:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){3}(((:[0-9A-Fa-f]{1,4}){1,4})|((:[0-9A-Fa-f]{1,4}){0,2}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){2}(((:[0-9A-Fa-f]{1,4}){1,5})|((:[0-9A-Fa-f]{1,4}){0,3}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(([0-9A-Fa-f]{1,4}:){1}(((:[0-9A-Fa-f]{1,4}){1,6})|((:[0-9A-Fa-f]{1,4}){0,4}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:))|(:(((:[0-9A-Fa-f]{1,4}){1,7})|((:[0-9A-Fa-f]{1,4}){0,5}:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)){3}))|:)))(%.+)?\s*$/;
        var isValidHost = hostNameRule.test( val ) && !hostNameRule2.test(val); 
        var isValidIp = cpVTypes_ipv4Rule.test( val ); // Consider removing  'isValidIp' - not necessarilly relevant
        var isValidIpv6 = hostNameRule3.test ( val );
        if( isValidHost == false && isValidIp == false && isValidIpv6 == false){ 
            return false;
        }
        else{
            return true;
    ...
    hostnameIPv6Text: 'Name must consist of 63 alphanumerics, numbers, "-", and "."',
    hostnameIPv6Mask: /[:\.\-a-zA-Z0-9]/,

	
    /* ~~~ @@ Domainname
     * Domainname labels may contain only the ASCII letters a-z, A-z, the digits 0-9, and the hyphen ('-'). 
     * Labels could not start with a hyphen, and must not end with a hyphen. 
     * Each label must be between 1 and 63 characters long, and the entire doaminname (including the delimiting dots) has a maximum of 255 characters.
     * Permitted doaminname labels to start with digits. 
     * No other symbols, punctuation characters, or white space are permitted.
     * Domainname field can also contain an IPv4 address.
     * e.g. comp.checkpoint.com, 172.16.254.1
     */
    domainname: function( val ){
        var domainNameRule = /^([a-zA-Z0-9]{0,64}|([a-zA-Z0-9](((\.)|(\-)|[a-zA-Z0-9]){0,62})[a-zA-Z0-9])){1,1}$/;
        var isValidDomain = domainNameRule.test( val ) ;
    ...
    domainnameText: 'Name must consist of 64 alphanumerics, numbers, "-", and "."',
    domainnameMask: /[\.\-a-zA-Z0-9_]/,

	
    
   /* ~~~ @@ IPv4 Address (simple text field)
    ...
    /* ~~~ @@ IPv4 Field (with 4 octets and 3 dots)
     * IPv4 addresses are represented in dot-decimal notation, 
     * which consists of four decimal numbers, each ranging from 0 to 255, separated by dots.
     * e.g. 172.16.254.1
     */
    ipv4field: function( val, field ){
        var fieldContainer = field.ownerCt;
        var fieldVal = fieldContainer.getValueFromOctets();
        var isValid = cpVTypes_ipv4Rule.test( fieldVal );
        if( isValid == true ){
            field.clearInvalid();
        }
        return isValid;
    },
    ipv4fieldText: 'Octet field is required',
     ...
     * Another restriction on the mac address is that we cannot set multicast address
     * multicast addresses has odd number in the MSB ( lsb of higher byte is 1 )
     */
    mac: function( val ){
        var macRule = /^[0-9a-fA-F][0|2|4|6|8|A|C|E|a|c|e][:-]([0-9a-fA-F]{2}[:-]){4}[0-9a-fA-F]{2}$/i;
        return macRule.test( val );
    },
    macText: '0Y:0:0:0:0:0 - FY:FF:FF:FF:FF:FF where Y is even hexadecimal number',
    macMask: /[a-fA-F0-9\:]/i,
    ...
    /* ~~~ @@ Groupname
     * Groupname can contains alphanumeric and/or numbers, with a maximum length of 8 characters.
     * e.g. group1
     */
    groupname: function( val ){
        var unRule = /^[0-9a-zA-Z_]{1,16}$/;
        return unRule.test( val );
    },
    groupnameText: 'Group Name may contain letters, numbers and underscore characters only, and cannot exceed 16 characters.',
    groupnameMask: /[0-9a-zA-Z_]/,
    ...
    username: function( val ){
        var unRule = /^[a-zA-Z][-_a-zA-Z0-9]{0,31}$/;
        return unRule.test( val );
    },
    usernameText: 'Username must begin with a letter and cannot exceed 32 characters. Please use alphanumerics, "-" ,and "_"',
    usernameMask: /[-_a-zA-Z0-9]/,
     ...
     * e.g. g670-U9
     * 
     * there many password rules for the different fields so curenttly it has been disabled
     */
    password: function( val ){
        //var pwdRule = /^([a-zA-Z0-9!@#\$%\^&\*\(\)\-_=\+:;].{5,18})$/;
        return true; //pwdRule.test( val );
    },
     ...
     */
    email: function( val ){
        var emailRule = /^[a-zA-Z]([a-zA-Z0-9]|[_\-\.]{1}[a-zA-Z0-9])+@[a-zA-Z0-9]([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,255})$/;
    ...
    /* ~~~ @@ Directory Path
     * Must start with slash (/) then alfanumeric, '.', '-' or '_'
     * e.g. /home/user-dir2/ddd.dd
     */
    path: function( val ){
        var pathRule = /^(\/[a-zA-Z0-9]([_\-\.a-zA-Z0-9])*)+$/;
        return pathRule.test( val );
    },
    pathText: 'Home Directory must start with a slash followed by alphanumeric, dot, hyphen or underscore characters',
    pathMask: /[-_.a-zA-Z0-9\/]/,

	
    /* ~~~ @@ Remote Path
     * Must start with slash (/) then alfanumeric, '.', '-' or '_' and end with a slash
     * e.g. /home/user-dir2/
     */
    remotepath: function( val ){
        var pathRule = /(^(\/)$)|(^(?:\/)(?:[a-zA-Z]|\d|\.|\/|_|-)*\/$)/;
        return pathRule.test( val );
    },
    remotepathText: 'Remote Directory must start with a slash followed by alphanumeric, dot, hyphen or underscore characters and end with a slash',
    remotepathMask: /[-_.a-zA-Z0-9\/]/	
